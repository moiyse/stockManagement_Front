...
const [stars,setStars]= useState(0)
    ...
    const getProduct = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/products/prod/${id}`);
    ...
    useEffect(() => {
      if (product && product.stars) { // check if product and stars exist
...
<div>
                ...
                <div className='row'>
                  ...
                  */}
                    ...
                    <div className="col-lg-6">
                        ...
                        <a href="#!" className="mb-4 d-block">  </a>
  ...
  <>
    ...
    {stars % 1 !== 0 && (
      <i key={Math.floor(stars) + '-half'} className="bi bi-star-half text-warning"></i>
    )}
...
)} {stars}<a href="#" className="ms-2">({product.nbReviewers} reviews)</a>
                        </div>
                        <div className="fs-4">
                            <span className="fw-bold text-dark">{product.reduction} DT  </span>
                            <span className="text-decoration-line-through text-muted">  {product.price} DT</span><span></span>
                        </div>
                        <hr className="my-6" />
                        {/*<div className="mb-4">
...
<small className="ms-3">{product.nbReviewers} global ratings</small></span>
                          </div>
                             {/* xx
                          <div className="mb-8">
  ...
  {[...Array(5 - Math.floor(reviews.rating))].map((_, index) => (
    <i key={Math.floor(reviews.rating) + index} className="bi bi-star text-warning"></i>
                        ...
                        <div>
                          {/* rating */}
                          <h3 className="mb-5" id="myDiv">Create Review</h3>
